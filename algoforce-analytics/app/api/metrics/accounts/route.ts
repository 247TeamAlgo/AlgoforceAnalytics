// app/api/metrics/[account]/route.ts
...
export async function GET(req: Request, ctx: { params: { account: string } }): Promise<NextResponse> {
    try {
        const id = (ctx.params.account || "").toLowerCase();
        const known = new Set((await readAccounts()).map(a => a.binanceName.toLowerCase()));
        if (!known.has(id)) return noStore(NextResponse.json({ error: "unknown account" }, { status: 400 }));
