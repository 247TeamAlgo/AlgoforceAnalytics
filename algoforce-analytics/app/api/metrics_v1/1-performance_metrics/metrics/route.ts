// algoforce-analytics\app\api\metrics_v1\1-performance_metrics\metrics\route.ts
...
export async function GET(req: Request): Promise<NextResponse> {
  try {
    ...
    const lastNDays =
      startDate || endDate
        ? undefined
        : url.searchParams.get("lastNDays")
          ? Number(url.searchParams.get("lastNDays"))
          : undefined;
    const runDate =
      startDate || endDate
        ? undefined
        : url.searchParams.get("runDate") || undefined;
    ...
    const monitoredSet = new Set(
      all.filter((a) => a.monitored).map((a) => a.redisName)
    );
    const selected = input.filter((a) => monitoredSet.has(a));
    if (!selected.length) {
      return noStore(
        NextResponse.json(
          { error: "no valid monitored accounts found in selection" },
          { status: 400 }
        )
      );
    }
    ...
    const meta = makeMeta(
      tz,
      result.merged.daily_return_last_n_days.window_end
    );
